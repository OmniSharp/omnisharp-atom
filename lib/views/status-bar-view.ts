...
import { each, has, isEqual, trim } from 'lodash';
...
interface StatusBarState {
    ...
    isReady?: boolean;
    isError?: boolean;
    status?: IOmnisharpClientStatus;
}

function updateState(self: any, state: any) {
    each(Omni.viewModelStatefulProperties, x => {
        if (has(state, x)) {
            self[x] = state[x];
        }
    });
...
export class StatusBarElement extends HTMLElement implements WebComponent, IDisposable {
    ...
    public attachedCallback() {
        ...
        this._disposable.add(server.observe.model
            .subscribe(model => {
                const solutionNumber = SolutionManager.activeSolutions.length > 1 ? trim(server.model && (<any>server.model).index, 'client') : '';
    ...
    private _showOnStateItems() {
        if (this._diagnostics.style.display === 'none') { this._diagnostics.style.display = ''; }
        if (this._projectCount.projects.style.display === 'none') { this._projectCount.projects.style.display = ''; }
    }

    private _hideOnStateItems() {
        if (this._diagnostics.style.display !== 'none') { this._diagnostics.style.display = 'none'; }
        if (this._projectCount.projects.style.display !== 'none') { this._projectCount.projects.style.display = 'none'; }
    }
