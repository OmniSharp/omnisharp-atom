            ...
            cd.add(issueRequest
                .flatMap((position: TextBuffer.Point) =>
                    Omni.request(editor, solution => solution.signatureHelp({
                        ...
                        .delay(10) // wait for the closing handler to catch up.
                        .flatMap(response => {
...
class SignatureBubble implements Rx.IDisposable {
    ...
    public update(position: TextBuffer.Point, member: OmniSharp.Models.SignatureHelp) {
        ...
        var range = [[position.row - 2, position.column], [position.row - 2, position.column]];
        if (!this._marker) {
        ...
        if (member && !_.isEqual(member, this._member)) {
    ...
    private _updateMember(member: OmniSharp.Models.SignatureHelp) {
        ...
        this._element.innerText = member.Signatures.map(x => `${x.Name} - ${x.Label} - ${x.Documentation} - ${x.Parameters.map(f => f.Name) }`).join('#');
    }
